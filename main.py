...
@app.get("/", response_class=JSONResponse, summary="Environment info")
async def root() -> dict[str, Any]:
    """
    Return basic information about the LLM Output Firewall environment.

    Returns:
        Dict with name, version, description, and available endpoints.
    """
    return {
        "name": "LLM Output Firewall",
        "version": "1.0.0",
        "description": (
            "An OpenEnv-compliant RL environment for training AI agents to detect "
            "and filter toxic, hallucinated, biased, and adversarial LLM outputs."
        ),
        "tasks": 3,
        "endpoints": {
            "reset": "POST /reset",
